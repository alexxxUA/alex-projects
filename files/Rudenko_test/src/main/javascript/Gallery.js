(function(){
    ...
    Gallery.prototype.doSearch = function(query, moduleId) {
        ...
        // will return promise
        var promise = this._imageFinder.search(query, moduleId, this);
        
        promise.then(function(data){
            console.log(data);
            self._onSearchResultReady(data);
        })
    ...
    Gallery.prototype._onSearchButtonClick = function(e) {
        ...
        var moduleId = this._modulesSelectNode.value;
    ...
    Gallery.prototype._createInterface = function() {
        ...
        this._modulesSelectNode = document.createElement('select');

        for (var i = 0; i < this._searchModulesList.length; i++){
            var opt = document.createElement('option');
            opt.value = this._searchModulesList[i];
            opt.innerHTML = this._searchModulesList[i];
            this._modulesSelectNode.appendChild(opt);
        }

        this._controlsNode.appendChild(this._modulesSelectNode);
    ...
    Gallery.prototype._searchModulesList = ['static', 'flickr']
